    ...
    if kstart < kmax:
        ...
        while count < max_iter:
            ...
            def Q_function(lambdas_):
                ...
                # return -Q
                return 1/(Q - penalty*numpy.inner(lambdas_, lambdas_)) # ridge regression (penalised likelihood)
...
def calc_logBF_ind(v, w, z_score):
    ...
    '''
    r = w / (v + w)
    # fGWAS #this is for thesis and fGWAS
...
def mk_eqtl_lambdas(p_cluster, z_scores, W=[0.01, 0.1, 0.5]):
    initial_lambdas = [0.0] * p_cluster.annotations.shape[0]
    # out = calc_logbinom(p_cluster.ld_matrix.shape[0], postgap.Globals.kmax_eqtl, len(p_cluster.z_scores))
    out = calc_logbinom(1, postgap.Globals.kmax_eqtl, len(p_cluster.z_scores))
    ...
    if postgap.Globals.TYPE == 'binom' or postgap.Globals.TYPE == 'EM':
        ...
        def f_llk(lambdas_):
            ...
            for i in range(len(priors)):
                ...
                lsum = lsum + sumlog(logBFs[i] + numpy.log(priors[i]), numpy.log(1 - priors[i])) ### log(BF * Pr + 1 - Pr)

            # return 1/(lsum - penalty*numpy.inner(lambdas_, lambdas_)) # ridge regression (penalised likelihood)
            return -(lsum - penalty*numpy.inner(lambdas_, lambdas_)) # ridge regression (penalised likelihood)
